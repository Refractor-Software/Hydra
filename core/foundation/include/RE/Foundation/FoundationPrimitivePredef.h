...
/* NOTE(will) Default platform primitives. Violates C reserved naming because fuck you, this is internal
 *            and not to be used elsewhere.
 *            This technically isn't necessary with stdint.h and cstdint available, but done regardless
 *            to avoid dragging in unneeded header imports from the standard library if we already know
 *            the types (also good for C89 for which preserving API compatibility is a nice-to-have).
 */

/* TODO(will) Separate out into FoundationPrimitiveTypesPlatformBegin.h and
 *            FoundationPrimitiveTypesPlatformEnd.h so we can define in a controlled scope.
 */
