...
internal void
Win64_Gamepad_PushAxisIfChanged (ReInputQueue *queue, ReUint8 gamepadIndex, ReInputGamepadAxis axis, ReFloat32 previous, ReFloat32 current)
{
    if (previous == current)
    {
        return;
...
Win64_Gamepad_HandleDisconnect (ReInputQueue *queue, ReUint8 gamepadIndex, ReWin64GamepadState *state)
{
    for (ReUint32 buttonIndex = 0; buttonIndex < WIN64_GAMEPAD_BUTTON_MAP_COUNT; buttonIndex += 1)
    {
        ...
        {
            Win64_Gamepad_PushButtonEvent (queue, gamepadIndex, gWin64GamepadButtonMap[buttonIndex].key, RE_False);
        }
    }
}
    ...
    for (ReUint8 gamepadIndex = 0; gamepadIndex < RE_INPUT_GAMEPAD_COUNT; gamepadIndex += 1)
    {
        ...
        if (newButtons != state->buttons)
        {
            for (ReUint32 buttonIndex = 0; buttonIndex < WIN64_GAMEPAD_BUTTON_MAP_COUNT; buttonIndex += 1)
            {
                ...
                if (wasDown != isDown)
                {
                    Win64_Gamepad_PushButtonEvent (queue, gamepadIndex, gWin64GamepadButtonMap[buttonIndex].key, isDown);
                }
            }
        ...
        ReFloat32 previousLeftX       = Win64_Gamepad_NormalizeStickAxis (state->leftStickX,  XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
        ReFloat32 previousLeftY       = Win64_Gamepad_NormalizeStickAxis (state->leftStickY,  XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
        ReFloat32 previousRightX      = Win64_Gamepad_NormalizeStickAxis (state->rightStickX, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
        ReFloat32 previousRightY      = Win64_Gamepad_NormalizeStickAxis (state->rightStickY, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
        ReFloat32 previousLeftTrigger  = Win64_Gamepad_NormalizeTriggerAxis (state->leftTrigger,  XINPUT_GAMEPAD_TRIGGER_THRESHOLD);
        ReFloat32 previousRightTrigger = Win64_Gamepad_NormalizeTriggerAxis (state->rightTrigger, XINPUT_GAMEPAD_TRIGGER_THRESHOLD);

        ReFloat32 currentLeftX       = Win64_Gamepad_NormalizeStickAxis (xinputState.Gamepad.sThumbLX, XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
        ReFloat32 currentLeftY       = Win64_Gamepad_NormalizeStickAxis (xinputState.Gamepad.sThumbLY, XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
        ReFloat32 currentRightX      = Win64_Gamepad_NormalizeStickAxis (xinputState.Gamepad.sThumbRX, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
        ReFloat32 currentRightY      = Win64_Gamepad_NormalizeStickAxis (xinputState.Gamepad.sThumbRY, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
        ReFloat32 currentLeftTrigger  = Win64_Gamepad_NormalizeTriggerAxis (xinputState.Gamepad.bLeftTrigger,  XINPUT_GAMEPAD_TRIGGER_THRESHOLD);
        ReFloat32 currentRightTrigger = Win64_Gamepad_NormalizeTriggerAxis (xinputState.Gamepad.bRightTrigger, XINPUT_GAMEPAD_TRIGGER_THRESHOLD);

        Win64_Gamepad_PushAxisIfChanged (queue, gamepadIndex, ReInputGamepadAxis_LeftStickX,   previousLeftX,        currentLeftX);
        Win64_Gamepad_PushAxisIfChanged (queue, gamepadIndex, ReInputGamepadAxis_LeftStickY,   previousLeftY,        currentLeftY);
        Win64_Gamepad_PushAxisIfChanged (queue, gamepadIndex, ReInputGamepadAxis_RightStickX,  previousRightX,       currentRightX);
        Win64_Gamepad_PushAxisIfChanged (queue, gamepadIndex, ReInputGamepadAxis_RightStickY,  previousRightY,       currentRightY);
        Win64_Gamepad_PushAxisIfChanged (queue, gamepadIndex, ReInputGamepadAxis_LeftTrigger,   previousLeftTrigger,  currentLeftTrigger);
        Win64_Gamepad_PushAxisIfChanged (queue, gamepadIndex, ReInputGamepadAxis_RightTrigger,  previousRightTrigger, currentRightTrigger);
