...
Win64_Render_RecreateRtvs( void )
{
    ...
    for ( ReUint32 i = 0; i < WIN64_RENDER_FRAME_COUNT; i += 1 )
    {
        gWin64RenderSwapChain->lpVtbl->GetBuffer(
            gWin64RenderSwapChain, i, &IID_ID3D12Resource, (void **) &gWin64RenderBackBuffers[i] );

        gWin64RenderDevice->lpVtbl->CreateRenderTargetView(
            gWin64RenderDevice, gWin64RenderBackBuffers[i], NULL, handle );
...
Win64_Render_CreateSwapchain( HWND window )
{
    ...
    gWin64RenderRtvDescriptorSize =
        gWin64RenderDevice->lpVtbl->GetDescriptorHandleIncrementSize(
            gWin64RenderDevice, D3D12_DESCRIPTOR_HEAP_TYPE_RTV );
